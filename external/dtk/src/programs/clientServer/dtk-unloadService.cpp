...
#define PROGRAM_NAME  "dtk-unloadService"
...
int Usage(void)
{
  ...
  printf("  Usage: %s%s%s %sNAME%s | [%s--help%s|%s-h%s]\n"
	 "\n"
	 "  %s%s%s unloads the DTK service named %sNAME%s. The\n"
	 "environment variable %sDTK_CONNECTION%s may be set to the servers\n"
	 "address and port and override the default of %s%s%s.\n"
	 "\n",
...
int main(int argc, char **argv)
{
  ...
  return - client.unloadService(argv[1]);
}
