 ...
 */
/*********************************************************************
 This is a very simple IS900 DTK tracker service for is900-viswall.  It just uses this
	 ...
	 )
	{
	  ...
	  if(
	     ...
	     )
	    {
	      ...
	      //state[j][0] = (getFloat(&ptr[Y]) + 152.4f)/152.4f;
		  state[j][0] = (getFloat(&ptr[Y]) - 152.4f)/152.4f;
	      state[j][1] = (getFloat(&ptr[X]) + 91.44f)/152.4f;
	      //state[j][2] = -(getFloat(&ptr[Z]) - 114.3f)/152.4f;
		  state[j][2] = -(getFloat(&ptr[Z]) + 152.4f)/152.4f;
