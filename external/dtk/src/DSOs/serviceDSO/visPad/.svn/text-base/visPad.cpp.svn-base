...
class VisPad : public dtkService
{
public:
  VisPad(void);
  ~VisPad(void);
...
private:
  ...
  float joystick[4];
  unsigned char buttons[4], led;
  char hat[2];
};


VisPad::VisPad(void) :
  joystickShm(sizeof(float)*4, "gamepad"),
  hatShm(sizeof(char)*2, "hat"),
  buttonsShm(sizeof(unsigned char), "buttons"),
  ledShm(sizeof(unsigned char), "axisPad/led")
{
  ...
  if(fd < 0)
    dtkMsg.add(DTKMSG_ERROR,
	       "VisPad::VisPad() failed find a good"
