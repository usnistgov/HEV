...
// In addition to the above, ServiceSlaveTCP also connects to
// dtkClients to send command requests to the DTK server.

class DTKSERVERAPI ServiceClientTCP : public dtkService, public dtkTCPSocket
{
public:

  ServiceClientTCP(const char *addressPort);
  ~ServiceClientTCP(void);
  int serve(void);
  void stop(void);

 private:

  // The memory for this comes from dtkSocketList serverSocketList.
  char *addressPort;
};
