...
void main() {
    ...
    if (gl_Vertex.x == 0. || gl_Vertex.y == 0.) {
        ...
        if (gl_Vertex.x == 0. && gl_Vertex.z != 0) {
            ...
            vertNum      = int ( 3 + 5*gl_Vertex.y/VolumeSize.y );
            int planeNum = int ( 3 + 5*gl_Vertex.z/VolumeSize.z );

           if (iris_ClipPlaneEnabled[planeNum])
                {
                ...
                // Could do this:
                //   plane = vec4 (0.0);
                // or just short-circuit the vertex shader:
                gl_ClipVertex = gl_Position = Vo = vec4(0,0,0,1);
                return;
                }
        ...
        //
        //   Geometry is a six-vertex tristrip zigzagging    2---4
        //   to make a hexagon. (012) triangle should be    /|\  |\        _
        //   front-facing. Between front and 2nd vertex    0 | \ | 5
        //   01, 23 and 45 coincide and some tris are       \|  \|/   
        //   degenerate. Between 2nd and 3rd, each is        1---3
        //   unique. Between 3rd vertex and back, 02, 14 and 35 coincide.
        //
        //   Alternatively, the geometry can be a hexagon with the vertices
        //   ordered CW as the diagram indicates.
