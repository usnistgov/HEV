...
main()
{
  printf("hev-playAudioFiles temporarily removed\n");
}
...
//Declare variables necessary for channel volume control and command line arguments.

int keep_settings_constant; //0 = keep settings off, 1 = keep settings on
double l, r, current_left_channel_setting, current_right_channel_setting;
char* current_fade_setting;
char f[2];			//+f = left to right, -f = right to left
struct audioInfo **ais;

//----------------------------------------------------------------------------------
//Stores some data necessary to play the audio file
struct audioInfo{
	SF_INFO sf;
	float *ab;
};
//----------------------------------------------------------------------------------
static void loadFile(int i, char *argv[]){
	
     float *buffer;
     SNDFILE *sndfile ;
     SF_INFO sfinfo ;

     if (! (sndfile = sf_open (argv [1], SFM_READ, &sfinfo)))		//Open the sndfile 
	      puts (sf_strerror (NULL)) ;
	 
		  
     if (sfinfo.channels < 1 || sfinfo.channels > 2)				//Check if it is for two channels			
	      printf ("Error : channels = %d.\n", sfinfo.channels) ;
	

     int bufferSize = sfinfo.channels * sfinfo.frames;
     buffer = malloc(sizeof(float) * bufferSize);

     sf_read_float(sndfile, buffer, bufferSize);
     ais[i] = malloc(sizeof(*ais[i]));
     ais[i]->ab = buffer;
     ais[i]->sf = sfinfo;	
     sf_close(sndfile);
...
/*
     ...
     fflush(stdout);
*/
     alsa_dev = alsa_open (ais[i]->sf.channels, ais[i]->sf.samplerate);
     buf = ais[i]->ab;
     bufSize = ais[i]->sf.channels * ais[i]->sf.frames;
     
     //Make volumes of zero very tiny, crashes fading 
     if(l == 0)
		l = .01;
     ...
     //fade setting (+f and -f)
     if(strlen(f) == 2){
		if(strcmp("+f",f) == 0){
			double multDec = 1;
			double deltaDec = (double)1/ais[i]->sf.frames;
			double multInc = 1;
			double deltaInc = (1/r - 1)/ais[i]->sf.frames;
			for(j = 0; j < bufSize; j += 2){
				buf[j] *= multDec;
				buf[j + 1] *= multInc;
				multDec -= deltaDec;
			...
			double deltaDec = (double)1/ais[i]->sf.frames;
			double multInc = 1;
			double deltaInc = (1/l - 1)/ais[i]->sf.frames;
			for(j = 0; j < bufSize; j += 2){
				buf[j] *= multInc;
				buf[j + 1] *= multDec;
				multDec -= deltaDec;
     ...
     alsa_write_float (alsa_dev, buf, ais[i]->sf.frames, ais[i]->sf.channels);
...
{
  const char *alsaDefault="default";

  // original
  //const char * device = "plughw:0" ;

  // see: http://www.alsa-project.org/main/index.php/Asoundrc#Plugins

  // Works best on Dalek
  // const char * device = "plug:dmix" ;

  // Works best on Vail.cam.nist.gov
  // const char * device = "default" ;

  // See rundemo
  const char *device=getenv("ALSADEV");
  if (device == NULL) {
    //perror("env varable ALSADEV not defined");
    //exit(1);
    device=alsaDefault;
  }
...
#endif   /* End of COMMENT_OUT */

main(int argc, char **argv) {

	//make argv dynamically allocated
	char **temp = (char **)malloc(argc*sizeof(char *));
	int c;
	for(c = 0; c < argc; c++){
		temp[c] = (char *)malloc(strlen(argv[c])*sizeof(char));
		strcpy(temp[c], argv[c]);
	}
	argv = temp;

     int i, pid, args;
     int blocking_constant = 0;					//0 = non-blocking, 1 = blocking
     
     if (argc < 2) {
	  printf ("\nUsage : %s soundFile0 soundFile1 ...\n", argv [0]) ;

	  printf ("\n Reads interger 0 to n-1 on stdin, plays corresponding soundfile\n");
	  return(1);
     }
     //determines blocking or non-blocking
     if(strcmp(argv[1],"-b") == 0){
		blocking_constant = 1;
		
		int j;
		//update argv to only contain filenames
		for(j = 1; j < argc-1; j++){
			argv[j] = argv[j+1];
		}
     ...
     ais = malloc(sizeof(**ais) * (argc - 1));
     current_fade_setting = (char *)malloc(sizeof(char)*2);
     current_fade_setting[0] = '\0';
     
     //Load all current files into memory
     int j;
     for(j = 0; j < argc - 1; j++)
     	loadFile(j, argv+j);
     ...
     while(getline(&line, &length, stdin) != -1) {   
     	...
     	if(line[0] == '-'){
     		switch(line[1]){
     		case 'b':
     			blocking_constant = (blocking_constant+1)%2;
     			break;
     		case 'c':
				...
				char* file_line = (char*)malloc(sizeof(char*));
				size_t line_len = 0;
	
				//read in the line of file names
				getline(&file_line, &line_len, input_pointer);
				argc = atoi(strtok(file_line, ", ")) + 1;				
				
				char* word;
				argv = (char **)realloc(argv, argc*sizeof(char *));
				for(c = 1; c < argc; c++){
					word = strtok(NULL, ", ");
					argv[c] = (char *)malloc(sizeof(char)*strlen(word));
					strcpy(argv[c], word);
				}
				
				//blocking	
				getline(&file_line, &line_len, input_pointer);
				blocking_constant = atoi(file_line);
				
				//channel control
				getline(&file_line, &line_len, input_pointer);
				
				current_left_channel_setting = atof(strtok(file_line, " "));
				current_right_channel_setting = atof(strtok(NULL, " "));
				
				//fading
				word = strtok(NULL, " ");				
				if(word != NULL){					
					strcpy(current_fade_setting, word);
				}
				
				keep_settings_constant = 1;
			...
			//remove the first 3 chars of line, which are the '-a '
			int count, len;
     			len = strlen(line)-3;		
			...
			line[len - 1] = '\0'; //mark the new end of line
     			argv[argc-1] = (char *)malloc(len*sizeof(char));
     			
     			//copy over to argv
     			strcpy(argv[argc-1],line);
     			
     			//load new audio file
     			loadFile(argc - 2, argv+argc-2); 
     			
     			break;
     		case 'k':
     			keep_settings_constant = (keep_settings_constant+1)%2;
     			printf("keep_settings_constant: %d\n", keep_settings_constant);
     			break;
     		}
	   ...
	   args = sscanf(line,"%d %lf %lf %s", &i, &l, &r, &f); 			//read in the command-line arguments
	   switch(args){ 								//choose appropriate action based on # of command-line arguments
	   	case 1:
	   		if(keep_settings_constant == 0){
		   		l = 1;
				r = 1;
				f[0] = '\0';
	   		}
	   		else{
	   			l = current_left_channel_setting;
	   			r = current_right_channel_setting;
	   			strcpy(f, current_fade_setting);
	   			
	   			printf("%f %f %s\n", current_left_channel_setting, current_right_channel_setting, current_fade_setting);
	   		}
			break;
	  	case 2:
	  		if ((l<0) || (l>1)) {
				fprintf(stderr,"Invalid l input: 0 <= l <= 1\n");
				continue;
			}
			r = (1 - l);
			f[0] = '\0';
		...
		case 3:
			if ((l<0) || (l>1) || (r<0) || (r>1)) {
				fprintf(stderr,"Invalid l/r input: 0 <= l/r <= 1\n");
				continue;
			}
			f[0] = '\0';
		...
		case 4:
			...
			if (!(strcmp(f,"+f") == 0 || strcmp(f,"-f") == 0)) {
				fprintf(stderr,"Invalid f input:\n+f, fade left to right\n-f, fade right to left\n");
				continue;
			}
		...
		default:
	   	...
	   	current_left_channel_setting = l;
		current_right_channel_setting = r;
		strcpy(current_fade_setting, f);
		
	   if ((i<0) || (i>=argc-1)) {
	   	fprintf(stderr,"Invalid i input: 0 <= i < n, number of audio clips\n",i);
		continue;
	   } 
	   
	   pid=fork();

	   if (pid == 0) {					//if it is the child, play the audio file
#if defined (__linux__)
#if HAVE_ALSA_ASOUNDLIB_H
	       if (access ("/proc/asound/cards", R_OK) == 0)
		    alsa_play (i) ;
		...
		if(blocking_constant == 1)			//if blocked, wait until finished playing before playing next
			while(wait(0) > 0);
	  };
