...
bool getImageDisplayOptions(std::string* envvars, std::string* options)
{
    ...
    // OK, we are now hardcoding the DISPLAY names based on which type
    // of immersive or desktop system we're on.  This is based on an
    // understanding of the display names for the immersive screens.
    // Note that if we're on the desktop, we use DISPLAY if it's
    // available and "0.0" if it is not.
    ...
    else if (ret == 1<<8) 
    {
        dtkMsg.add(DTKMSG_ERROR, 
            "hev-presenter::getImageDisplayOptions: unknown system type returned\n") ;
        return false ;
    }
    else if (ret == -1)
    {
    ...
    {
        dtkMsg.add(DTKMSG_ERROR, 
          "hev-presenter::getImageDisplayOptions: unknown return value from system call\n") ;
        return false ;
    }
