...
void usage()
{
  fprintf(stderr,"Usage: hev-uniformSlider node uniform [val min max]\n");
}
...
bool init(int argc, char **argv)
{
  ...
  // system( "hev-applyState -s fatpoint.osg $s\n", node );
...
void updateVal(float f)
{
  ...
  xSlider->value( f ) ;             // Set slider value.
...
void updateMin(float f)
{
  ...
  xSlider->minimum( xmin );         // Set slider minimum.
...
void updateMax(float f)
{
  ...
  xSlider->maximum( xmax );         // Set slider maximum.
  updateVal( xval );                // Update value to insure it is clamped
}
