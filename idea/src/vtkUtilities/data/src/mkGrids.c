...
static int
writeRCoords (FILE *fp, char dir, int dim, int reverse, double coords[])
        {
        int i;
...
static void
writeRGHeader  (FILE *fp, int dim[3], char *desc, double coords[3][1000])
        {
        ...
        fprintf (fp, "DIMENSIONS %d %d %d\n", dim[0], dim[1], dim[2]  );
        writeRCoords (fp, 'X', dim[0], 1, coords[0]);
        writeRCoords (fp, 'Y', dim[1], 0, coords[1]);
        writeRCoords (fp, 'Z', dim[2], 1, coords[2]);
  ...
  int noise
        ...
        for (k = 0; k < dim[2]; k++)
            ...
            for (j = 0; j < dim[1]; j++)
                ...
                for (i = 0; i < dim[0]; i++)
                        {
                        int iii = (axis == 0) ? i : (axis == 1) ? j : k;
                        double val = coord[iii];
                        if (noise)
                                {
                                addNoise (0.005, &val);
                                }
...
static int
writeSPCoord (char *fn, int dim[3], int axis, int noise)
        {
        FILE *fp;
        char desc[1000];
        double coord[1000];
        ...
        writeSPHeader (fp, dim, "  ");

        
        for (i = 0; i < dim[axis]; i++)
                {
                coord[i] = i;
                }
        writeGridDataCoord (fp, dim, axis, coord, noise);
...
writeRG (char *fn, int dim[3], double center[3], int noise)
        ...
        sprintf (desc, 
          "RG Distorted Ellipsoid: dims %d %d %d  center %g %g %g  noise %s",
                dim[0], dim[1], dim[2], center[0], center[1], center[2],
                noise?"yes":"no");
        writeRGHeader (fp, dim, desc, coords);
        writeGridData (fp, dim, center, noise);
...
static int
writeRGCoord (char *fn, int dim[3], int axis, int noise)
        {
        FILE *fp;
        char desc[1000];
        double coords[3][1000];
        ...
        writeRGHeader (fp, dim, "", coords);
        writeGridDataCoord (fp, dim, axis, coords[axis], noise);
...
main ()
        ...
        writeSPCoord ("structPoints.coordData.X.vtk", dim, 0, 0);
        writeSPCoord ("structPoints.coordData.Y.vtk", dim, 1, 0);
        writeSPCoord ("structPoints.coordData.Z.vtk", dim, 2, 0);
        writeRGCoord ("rectGrid.coordData.X.vtk", dim, 0, 0);
        writeRGCoord ("rectGrid.coordData.Y.vtk", dim, 1, 0);
        writeRGCoord ("rectGrid.coordData.Z.vtk", dim, 2, 0);

        dim[0] = 5;
        dim[1] = 5;
        dim[2] = 5;
        center[0] = center[1] = center[2] = 0;
        writeSP ("sphData.sp.1.vtk", dim, center, 0);
        centerInDim (dim, center);
        writeSP ("sphData.sp.2.vtk", dim, center, 0);
